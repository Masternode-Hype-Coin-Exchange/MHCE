...
CzmhceTracker::CzmhceTracker(std::string strWalletFile)
{
...
CzmhceTracker::~CzmhceTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzmhceTracker::Init()
{
...
bool CzmhceTracker::Archive(CMintMeta& meta)
{
...
bool CzmhceTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzmhceTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzmhceTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzmhceTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzmhceTracker::GetSerialHashes()
{
...
CAmount CzmhceTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzmhceTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzmhceTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzmhceTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzmhceTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzmhceTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzmhceTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzmhceTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzmhceTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzmhceTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzmhceTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmhceTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmhceTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzmhceTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzmhceTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzmhceTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzmhceTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmhce from DB\n", __func__, listDeterministicDB.size());
    }
