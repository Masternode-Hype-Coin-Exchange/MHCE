...
public:
    ...
    bool WriteZMHCESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMHCESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMHCESeed_deprecated(uint256& seed);
    bool EraseZMHCESeed();
    bool EraseZMHCESeed_deprecated();

    bool WriteZMHCECount(const uint32_t& nCount);
    bool ReadZMHCECount(uint32_t& nCount);
