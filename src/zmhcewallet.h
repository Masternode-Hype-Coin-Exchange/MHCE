...
#ifndef MasternodeHypeCoinExchange_ZMHCEWALLET_H
#define MasternodeHypeCoinExchange_ZMHCEWALLET_H
...
public:
    CzmhceWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMHCE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMHCE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MasternodeHypeCoinExchange_ZMHCEWALLET_H
