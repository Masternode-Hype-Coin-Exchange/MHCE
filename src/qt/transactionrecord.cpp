        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by MasternodeHypeCoinExchange Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to MasternodeHypeCoinExchange Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZMHCEType(TransactionRecord::Type type)
{
    switch (type) {
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZMHCE || type == TransactionRecord::MNReward) {
