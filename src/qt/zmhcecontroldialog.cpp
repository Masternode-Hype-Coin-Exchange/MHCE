...
#include "ui_zmhcecontroldialog.h"
...
std::set<std::string> ZMHCEControlDialog::setSelectedMints;
std::set<CMintMeta> ZMHCEControlDialog::setMints;

ZMHCEControlDialog::ZMHCEControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMHCEControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMHCEControlDialog::~ZMHCEControlDialog()
{
    delete ui;
}

void ZMHCEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMHCEControlDialog::updateList()
{
...
void ZMHCEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMHCEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMHCE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMHCEControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMHCEControlDialog::GetSelectedMints()
{
...
void ZMHCEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
