...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MHCE);
    ...
    if (!settings.contains("nAnonymizeMasternodeHypeCoinExchangeAmount"))
        settings.setValue("nAnonymizeMasternodeHypeCoinExchangeAmount", 1000);

    nAnonymizeMasternodeHypeCoinExchangeAmount = settings.value("nAnonymizeMasternodeHypeCoinExchangeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMasternodeHypeCoinExchangeAmount"))
        SoftSetArg("-anonymizemhceamount", settings.value("nAnonymizeMasternodeHypeCoinExchangeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mhce.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasternodeHypeCoinExchangeAmount:
            return QVariant(nAnonymizeMasternodeHypeCoinExchangeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasternodeHypeCoinExchangeAmount:
            nAnonymizeMasternodeHypeCoinExchangeAmount = value.toInt();
            settings.setValue("nAnonymizeMasternodeHypeCoinExchangeAmount", nAnonymizeMasternodeHypeCoinExchangeAmount);
            emit anonymizeMasternodeHypeCoinExchangeAmountChanged(nAnonymizeMasternodeHypeCoinExchangeAmount);
            break;
        case CoinControlFeatures:
