...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MasternodeHypeCoinExchange address (e.g. %1)").arg("C6M1kT6PQaieDqNv2ajcTJtcfip2krta6a"));
...
{
    // return if URI is not valid or is no MasternodeHypeCoinExchange: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::MHCE, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert mhce:// to mhce:
    //
    //    Cannot handle this later, because mhce:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MHCE, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open mhce.conf with the associated application */
...
int TableViewLastColumnResizingFixer::getColumhceWidth()
{
    int nColumhceWidthSum = 0;
    for (int i = 0; i < columnCount; i++) {
        nColumhceWidthSum += tableView->horizontalHeader()->sectionSize(i);
    }
    return nColumhceWidthSum;
}
...
int TableViewLastColumnResizingFixer::getAvailableWidthForColumn(int column)
{
    ...
    if (nTableWidth > 0) {
        int nOtherColsWidth = getColumhceWidth() - tableView->horizontalHeader()->sectionSize(column);
...
// Make sure we don't make the columhce wider than the tables viewport width.
void TableViewLastColumnResizingFixer::adjustTableColumhceWidth()
{
    ...
    int nTableWidth = tableView->horizontalHeader()->width();
    int nColsWidth = getColumhceWidth();
...
{
    if ((getColumhceWidth() - this->tableView->horizontalHeader()->width()) != 0) {
        disconnectViewHeadersSignals();
        resizeColumn(secondToLastColumnIndex, getAvailableWidthForColumn(secondToLastColumnIndex));
        connectViewHeadersSignals();
    }
 ...
 * the resize modes of the last 2 columhce of the table and
 */
TableViewLastColumnResizingFixer::TableViewLastColumnResizingFixer(QTableView* table, int lastColMinimumWidth, int allColsMinimumWidth) : tableView(table),
                                                                                                                                          ...
                                                                                                                                          allColumhceMinimumWidth(allColsMinimumWidth)
{
    columnCount = tableView->horizontalHeader()->count();
    lastColumnIndex = columnCount - 1;
    secondToLastColumnIndex = columnCount - 2;
    tableView->horizontalHeader()->setMinimumSectionSize(allColumhceMinimumWidth);
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MasternodeHypeCoinExchange.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MasternodeHypeCoinExchange.lnk
...
{
    return GetAutostartDir() / "mhce.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mhce.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MasternodeHypeCoinExchange\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mhce app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mhce app to startup item list
