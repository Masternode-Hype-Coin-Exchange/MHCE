...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mhce.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mhced.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MHCE and zmhce money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MHCE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MHCE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mhce, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MHCE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MasternodeHypeCoinExchange specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-MHCEstake=<n>", strprintf(_("Enable or disable staking functionality for MHCE inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 37266, 32823));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		   ...
		   "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The The Masternode Hype Coin Exchange Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that MasternodeHypeCoinExchange is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasternodeHypeCoinExchange is shutting down."));
...
#endif
    // Make sure only a single MasternodeHypeCoinExchange process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasternodeHypeCoinExchange is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasternodeHypeCoinExchange version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MasternodeHypeCoinExchange: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MasternodeHypeCoinExchange specific: zerocoin and spork DB's
                ...
                // MasternodeHypeCoinExchange: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasternodeHypeCoinExchange") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MasternodeHypeCoinExchange to complete") << "\n";
        ...
        zwalletMain = new CzmhceWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zmhceWallet


        //Load zerocoin mint hashes to memory
        pwalletMain->zmhceTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeMasternodeHypeCoinExchangeAmount = 2;
...
//    }
//
//    nAnonymizeMasternodeHypeCoinExchangeAmount = GetArg("-anonymizemhceamount", 0);
//    if (nAnonymizeMasternodeHypeCoinExchangeAmount > 999999) nAnonymizeMasternodeHypeCoinExchangeAmount = 999999;
//    if (nAnonymizeMasternodeHypeCoinExchangeAmount < 2) nAnonymizeMasternodeHypeCoinExchangeAmount = 2;
    ...
    LogPrintf("AnonymizeMasternodeHypeCoinExchangeAmount %d\n", nAnonymizeMasternodeHypeCoinExchangeAmount);
    ...
    /* Denominations
       ...
       For example:
       1MHCE +1000 == (.1MHCE +100)*10
       10MHCE +10000 == (1MHCE +1000)*10
    */
