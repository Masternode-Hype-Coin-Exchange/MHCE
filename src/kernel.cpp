...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMHCEStake(spend));
    } else {
        ...
        CMHCEStake* MHCEInput = new CMHCEStake();
        MHCEInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(MHCEInput);
    }
