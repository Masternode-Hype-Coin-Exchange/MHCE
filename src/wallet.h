...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 MHCE at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zmhce send
enum ZerocoinSpendStatus {
    ZMHCE_SPEND_OKAY = 0,                            // No error
    ZMHCE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMHCE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMHCE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMHCE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMHCE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMHCE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMHCE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMHCE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMHCE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMHCE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMHCE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMHCE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMHCE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMHCE_SPENT_USED_ZMHCE = 14,                      // Coin has already been spend
    ZMHCE_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMHCE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZMHCEBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMHCEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmhceAuto) const;
    ...
    */
    ...
    CzmhceWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzmhceTracker> zmhceTracker;
    ...
    void setZWallet(CzmhceWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmhceTracker = std::unique_ptr<CzmhceTracker>(new CzmhceTracker(strWalletFile));
    }

    CzmhceWallet* getZWallet() { return zwalletMain; }
    ...
    /** zmhce reset */
    boost::signals2::signal<void()> NotifyzmhceReset;
